...
- brief invariants;
- current route/stage;
- unlocked mutation classes;
- candidate parentage and concrete operator;
- render artifacts;
- validity and adherence review;
- pairwise visual/temporal preference;
- stage-unlock reasons;
- elite promotion history.

The CLI enforces important guardrails: locked mutation classes cannot be added, stage unlocks are sequential, filament topology change requires an explicit brief change, experimental morphology body-plan search requires an explicit experimental flag, and a candidate cannot be promoted without passing validity, adherence and visual/temporal preference.
