...
from candidate_review_scheduler import ReviewProposal
...
class EvidenceAuthoritySelector(PairwiseSelector):
    ...
    The default remains eager pair-queue creation. ``collect_review_proposals`` is
    ...
    def __init__(self,*,render_frame,times:Sequence[float],evidence_dirs:Sequence[Path]=(),queue_dir:Optional[Path]=None,advisory:Optional[PairwiseSelector]=None,max_pending_reviews:Optional[int]=None,max_pending_reviews_per_group:Optional[int]=None,extra_evidence:Sequence=(),collect_review_proposals:bool=False):
        ...
        self.collect_review_proposals=bool(collect_review_proposals)
    ...
    def _queue(self,*,pair_id,brief_text,a_frames,b_frames,a_id,b_id,review_group)->bool:
        ...
        return True
    def _collect_proposal(self,*,pair_id,brief_text,a,b,afp,bfp,a_frames,b_frames,review_group)->bool:
        if pair_id in self._proposal_pair_ids or pair_id in self.pending_review_ids:
            return False
        self._proposal_pair_ids.add(pair_id)
        self.review_proposals.append(ReviewProposal(
            index=len(self.review_proposals),
            pair_id=pair_id,
            ...
            afp=afp,
            bfp=bfp,
            a_frames=tuple(a_frames),
            b_frames=tuple(b_frames),
            a_route=str(getattr(a,'route','') or ''),
            b_route=str(getattr(b,'route','') or ''),
            a_stage=str(getattr(a,'stage','') or ''),
            b_stage=str(getattr(b,'stage','') or ''),
            a_parent=getattr(a,'parent_id',None),
            b_parent=getattr(b,'parent_id',None),
    ...
    def compare(self,a,b,brief:Mapping[str,object])->PairwiseDecision:
        ...
        queued=False; collected=False; review_group=self._review_group(a,b,brief)
        if resolution.review_needed:
            if self.collect_review_proposals:
                collected=self._collect_proposal(
                    pair_id=pair_id,brief_text=brief_text,a=a,b=b,afp=afp,bfp=bfp,
                    a_frames=a_frames,b_frames=b_frames,review_group=review_group,
                )
            else:
        ...
        if resolution.review_needed and self.collect_review_proposals:
            if collected:
                reason+='; review proposal collected for post-search scheduling'
            elif pair_id in self.pending_review_ids:
                reason+='; review is already pending'
            else:
                reason+='; review proposal was already observed in this replay'
        elif resolution.review_needed and self.queue_dir is not None and not queued:
