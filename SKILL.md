...
For multi-round search, also load `references/discovery-state.md` and use `scripts/discovery-state.mjs` to track the elite, brief invariants, unlocked mutation classes and lineage. The state is bookkeeping; it must not become an aesthetic scorer.
...
### Discovery notes
State the incumbent, which search stages were explored/unlocked, and why the winner survived selection. When a discovery-state file was used, summarize its elite lineage rather than inventing a separate history.
