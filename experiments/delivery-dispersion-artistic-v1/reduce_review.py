...
def score(judgment: str, a_policy: str, b_policy: str) -> int:
    if judgment in {"equivalent", "unreviewable"}:
        return 0
    winner = "A" if judgment == "A>B" else "B"
    winner_policy = a_policy if winner == "A" else b_policy
    return 1 if winner_policy == "dispersion" else -1
...
def reduce_review(ratings_path: str, key_path: str) -> dict:
    ...
    for block in sorted(blocks, key=lambda x: x["blockId"]):
        ...
        rows.append({
            ...
            "A": block["A"],
            "B": block["B"],
            "dispersionVsQuantileScore": value,
        })
    ...
    gates = {
        ...
        "totalDispersionVsQuantileNetPositive": total_net > 0,
    ...
    return {
        ...
        "totalDispersionVsQuantileNetPreference": total_net,
