...
def generate(summary_path: Path, evidence_dir: Path, output_root: Path) -> dict:
    ...
    if winning_arm not in ('memoryHybrid60', 'meanHybrid60'):
        skipped = {
            'skipped': True,
            'reason': 'no mechanically promising hybrid arm',
    ...
    for concept in prompts:
        ...
        route = arm['route']
        genome = arm['genome']
        cand = ld.quick_candidate(route, genome, f'hybrid-recognition-{concept}-{seed}')
