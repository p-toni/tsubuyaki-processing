...
REVIEW_SEEDS = (756003, 756019, 756037, 756053, 756071, 756089, 756107, 756127)
SMOKE_SEED = 756999
TIMES = (30, 90, 150)
QUANTILES = (0.35, 0.75)
MIN_VALID_GENERATED = 12
BLIND_SALT = "semantic-judge-prospective-v1-replacement-20260901"
THUMB = 180
PAPER = (250, 248, 245)
INK = (42, 40, 36)
LINE = (235, 230, 221)
...
def _brief(route: str) -> dict:
    return {
        "name": "semantic-judge-prospective-v1-replacement",
...
def generate(output_root: Path, smoke: bool) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="semantic-judge-review-replacement-") as td:
        temp = Path(td)
        for seed in seeds:
            for route in ROUTES:
                seq += 1
    ...
    contract = {
        "version": 1,
        "experiment": "semantic-judge-prospective-v1-replacement",
        "smoke": smoke,
    ...
    key = {"version": 1, "experiment": "semantic-judge-prospective-v1-replacement", "smoke": smoke, "blindSalt": BLIND_SALT, "blocks": key_blocks}
