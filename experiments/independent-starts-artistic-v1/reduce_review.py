...
def reduce_review(ratings_path: str, key_path: str) -> dict:
    ...
    for block in sorted(blocks, key=lambda x: x["blockId"]):
        ...
        rows.append({
            ...
            "A": block["A"],
            "B": block["B"],
            "restartTailVsBaselineScore": value,
        })
    ...
    gates = {
        ...
        "totalRestartTailVsBaselineNetPositive": total_net > 0,
    ...
    return {
        "version": 1,
        "decision": (
            "INDEPENDENT_STARTS_ARTISTIC_SUPPORT"
            if passed
            else "INDEPENDENT_STARTS_ARTISTIC_SUPPORT_NOT_DEMONSTRATED"
        ),
        "blockCount": 12,
        "reviewableCount": reviewable,
        "judgmentCounts": dict(counts),
        "totalRestartTailVsBaselineNetPreference": total_net,
