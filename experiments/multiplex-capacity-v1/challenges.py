...
They encode structural demands, not copies of any public artwork.
"""
...
def _rng(seed: int, challenge: Challenge) -> random.Random:
    # Stable arithmetic namespace; no dependence on Python hash randomization.
...
def _linked(seed: int, challenge: Challenge, t: float, width: int, height: int):
    ...
    for lane in range(lanes):
        lane_phase = (lane - (lanes - 1) / 2.0) * rng.uniform(0.18, 0.42)
        lane_offset = (lane - (lanes - 1) / 2.0) * rng.uniform(8.0, 17.0)
        for j in range(samples):
            q = 2.0 * math.pi * j / samples
            phase = q + lane_phase + 0.10 * math.sin((2 + challenge.variant) * q - t / rng.uniform(21.0, 37.0))
            radial = base * (1.0 + lobe * math.sin((2 + lane % 2) * q + t / rng.uniform(24.0, 46.0)))
...
def _woven(seed: int, challenge: Challenge, t: float, width: int, height: int):
    ...
    for j in range(n):
        s = j / max(1, n - 1)
        q = 2.0 * math.pi * s
        carrier = math.sin((a + 0.35) * q - t / rng.uniform(17.0, 31.0))
        weave = math.sin(b * q + 0.55 * math.sin(2 * q) + t / rng.uniform(31.0, 49.0))
...
def _coupled(seed: int, challenge: Challenge, t: float, width: int, height: int):
    ...
    for j in range(n):
        ...
        radial = base * (0.78 + rng.uniform(0.18, 0.30) * field + 0.08 * math.cos((lobes + 2) * q))
        motion = field * rng.uniform(8.0, 17.0) * math.sin(t / rng.uniform(13.0, 27.0) + 2.0 * q)
...
def _detail(seed: int, challenge: Challenge, t: float, width: int, height: int):
    ...
    for iv in range(nv):
        ...
        for iu in range(nu):
            ...
            ridge = math.sin((4 + challenge.variant) * u + 2.0 * v - t / rng.uniform(19.0, 37.0))
            bend = rng.uniform(0.06, 0.16) * math.sin(2.0 * v + t / 43.0)
            x = width / 2 + sx * (u + bend * (1 - u * u)) + (8 + 4 * challenge.variant) * local * ridge
            y = height / 2 + sy * (v + rng.uniform(-0.05, 0.14) * u * u) + rng.uniform(10, 22) * local * math.sin(3 * u - t / 31.0)
            points.append((x, y))
    return points
