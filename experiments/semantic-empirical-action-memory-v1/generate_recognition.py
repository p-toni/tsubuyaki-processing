...
def generate(summary_path: Path, evidence_dir: Path, output_root: Path) -> dict:
    ...
    if winning_arm not in ('empiricalMemory60', 'meanDynamics60'):
        skipped = {'skipped': True, 'reason': 'no mechanically promising arm', 'decision': summary.get('decision')}
        (output_root / 'skipped.json').write_text(json.dumps(skipped, indent=2, sort_keys=True) + '\n')
        return skipped
    ...
    for concept in prompts:
        ...
        route = arm['route']
        genome = arm['genome']
        cand = ld.quick_candidate(route, genome, f'recognition-{concept}-{seed}')
    ...
    instructions = {
        ...
        'note': 'Spontaneous recognizability should also be recorded separately from forced-choice exact matching.',
    }
    (review_dir / 'instructions.json').write_text(json.dumps(instructions, indent=2, sort_keys=True) + '\n')
    (review_dir / 'README.txt').write_text(
        'Match panels A-H to the following labels, using each exactly once:\n\n'
        + ', '.join(prompts)
        + '\n\nDo not rate artistic quality; only identify the intended shape. If a panel is not spontaneously recognizable, note that before making any forced-choice guess.\n'
    )
    sealed = {
        'version': 1,
