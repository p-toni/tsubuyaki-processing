...
from targets_operator import build_targets_operator
...
def _valid_starts(master_seed: int, route: str) -> tuple[list[dict], int]:
    ...
    rng = representation_rng(master_seed, route, version, "spectral-material-control-operator-v1-starts")
    starts = []
    attempts = 0
...
def _native_challengers(master_seed: int, route: str, starts: list[dict]) -> list[dict]:
    ...
    for start_index, base in enumerate(starts):
        rng = representation_rng(
            ...
            route,
            version,
            f"spectral-material-control-operator-v1-native-{start_index}",
        )
...
def run_seed(master_seed: int) -> dict:
    ...
    # Generate every route archive before target construction/scoring.
    archives = {}
    for route in ROUTES:
    ...
    for route in ROUTES:
        ...
        for target in targets:
            ...
            spectral_recovery = max(
                [base_recovery] + [_recovery(image, target.image) for image in spectral_images]
            )
            cells.append(
                {
    ...
    hard = {
        ...
        "twoSharedStartsEveryRoute": all(
            route_diagnostics[r]["sharedValidStarts"] == STARTS_PER_ROUTE for r in ROUTES
        ),
        "nativeBudgetExact": all(
            route_diagnostics[r]["nativeAttempts"] == CHALLENGERS_PER_ARM for r in ROUTES
        ),
        "spectralBudgetExact": all(
            route_diagnostics[r]["spectralAttempts"] == CHALLENGERS_PER_ARM for r in ROUTES
        ),
